...
\hline
Systolic Blood Pressure & mmHg & 100 & 0.05 & 0.40 \\
\hline
Diastolic Blood Pressure & mmHg & 70 & 0.30 & 0.25 \\
\hline
...
Fig.~\ref{fig_cm} and Fig.~\ref{fig_roc} present the classification performance comparison using Confusion Matrix and ROC Curve analysis for different normalization methods. The confusion matrix results show that experiments without normalization produced a higher number of misclassifications, particularly in detecting positive Dengue Fever cases. In contrast, both \textit{StandardScaler} and \textit{MinMaxScaler} significantly improved prediction accuracy by reducing false classifications. Among all methods, \textit{StandardScaler} achieved the best classification results with the highest number of correctly predicted samples.

Similarly, the ROC Curve analysis demonstrates that normalization substantially improved the classification capability of the proposed Deep Neural Network model. The model without normalization achieved a relatively low AUC value of 0.6060, while \textit{StandardScaler} and \textit{MinMaxScaler} achieved significantly higher AUC values of 0.9972 and 0.9920, respectively. These findings confirm that normalization plays an important role in improving model sensitivity, specificity, and overall classification performance for Dengue Fever prediction.
...
Future studies may improve the proposed framework by utilizing larger and more diverse clinical datasets collected from multiple healthcare institutions. Additional experiments using more advanced Deep Learning architectures such as CNN, LSTM, or hybrid models may also enhance classification performance. Furthermore, future research can focus on integrating the proposed model into real-time healthcare applications or web-based systems to support early Dengue Fever screening and clinical decision-making processes.
